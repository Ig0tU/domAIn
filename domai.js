...
// @version 2025-06-23.8 // Enhanced version with improvements
// @description An incredibly powerful AI assistant that can interact with and automate any webpage using Google Gemini, featuring an extensive suite of 109 DOM manipulation, information retrieval, and browser control tools with enhanced UI and better error handling.
// @author k0880k
// @match *://*/*
// @grant GM_addStyle
    ...
    const assistantHTML = `
        <div class="dom-ai-assistant" id="domAiAssistant">
            <div class="dom-ai-header" id="domAiHeader">
                <div class="dom-ai-title">
                    <span>DOM-AI Assistant</span>
                    <span class="tool-counter" id="toolCounter">109 Tools</span>
                </div>
                <div class="dom-ai-header-buttons">
                    <button id="settingsBtn" title="Settings">⚙️</button>
                    <button id="listToolsBtn" title="List Available Tools">🛠️</button>
...
`;
    ...
    async function callGeminiApi() {
        if (!apiKey) {
            addMessage("API Key is not set. Please open settings (⚙️) and save your Gemini API key.", 'ai', false, 'tool-error');
            return null;
        }
    ...
    function init() {
        ...
        saveApiKeyBtn.addEventListener('click', () => {
            ...
            if (!inputKey) {
                addMessage("Please enter an API key.", 'ai', false, 'tool-error');
                return;
            }
            
            if (inputKey.length < 20) {
                addMessage("API key appears to be too short. Please check your key.", 'ai', false, 'tool-error');
                return;
            }
            
            if (!inputKey.startsWith('AI')) {
                addMessage("Warning: Gemini API keys typically start with 'AI'. Please verify your key is correct.", 'ai', false, 'tool-error');
            }
            
            apiKey = inputKey;
            GM_setValue('domAiApiKey', apiKey);
            settingsModal.style.display = 'none';
            addMessage("API Key saved successfully! I'm ready to help.", 'ai');
        });
        ...
        document.addEventListener('keydown', (e) => {
            // Only activate shortcuts when the assistant is visible and not in an input field
            if (assistant.style.display === 'none' || e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') {
                return;
            }

            // Ctrl/Cmd + Shift + D: Toggle assistant visibility
        ...
        if (apiKey) {
            addMessage("DOM-AI Assistant initialized with enhanced features! I'm ready to help!<br><br><strong>New Keyboard Shortcuts:</strong><br>• Ctrl+Shift+D: Toggle assistant<br>• Ctrl+Shift+M: Minimize/restore<br>• Ctrl+Shift+C: Clear chat<br>• Ctrl+Shift+T: Show tools<br>• Ctrl+Shift+I: Inspect mode<br>• Ctrl+Shift+F: Focus input", 'ai');
        } else {
            addMessage("Welcome to the enhanced DOM-AI Assistant! To get started, please set your Gemini API key in settings (⚙️).<br><br><strong>Keyboard Shortcuts Available:</strong><br>• Ctrl+Shift+D: Toggle assistant<br>• Ctrl+Shift+M: Minimize/restore<br>• Ctrl+Shift+C: Clear chat<br>• Ctrl+Shift+T: Show tools<br>• Ctrl+Shift+I: Inspect mode<br>• Ctrl+Shift+F: Focus input", 'ai');
            settingsModal.style.display = 'flex';
        }
    }

    /**
     * Updates the tool counter in the header to reflect the current number of available tools.
     */
    function updateToolCounter() {
        const toolCounter = document.getElementById('toolCounter');
        if (toolCounter) {
            const toolCount = toolDeclarations.length;
            toolCounter.textContent = `${toolCount} Tools`;
        }
    }
